...
- [Tseitin Encoding for Cross-Tree Constraints](#tseitin-encoding-for-cross-tree-constraints)
...
### Tseitin Mode (`-t` flag)
- Introduces **auxiliary variables** for cross-tree constraint sub-expressions
- Feature tree relation clauses (OR groups, ALTERNATIVE groups, CARDINALITY) are emitted **directly as CNF disjunctions** of arbitrary length — no decomposition needed
- Linear-size output for cross-tree constraint expressions
- **Preserves solution counts**: Uses full biconditional equivalences (⟺) for auxiliary variables
- Best for: Models with deeply nested Boolean cross-tree constraint expressions
...
| UVL Relation | Semantics | CNF Encoding (both modes) |
|--------------|-----------|--------------------------|
| **MANDATORY** | `parent ⟺ child` | `(¬parent ∨ child)` <br> `(¬child ∨ parent)` |
| **OPTIONAL** | `child → parent` | `(¬child ∨ parent)` |
| **OR** | `parent → (c₁ ∨ c₂ ∨ ... ∨ cₙ)` | `(¬parent ∨ c₁ ∨ c₂ ∨ ... ∨ cₙ)` <br> + `(¬cᵢ ∨ parent)` for each child |
| **ALTERNATIVE** | `parent → (exactly one of c₁...cₙ)` | `(¬parent ∨ c₁ ∨ ... ∨ cₙ)` [at least one] <br> `(¬cᵢ ∨ ¬cⱼ)` for all pairs [at most one] <br> + `(¬cᵢ ∨ parent)` for each child |
| **CARDINALITY [m..n]** | `parent → (select between m and n children)` | Enumeration-based: forbid invalid counts using combinations |

Note: Feature tree relation clauses are emitted directly in **both** STRAIGHTFORWARD and TSEITIN modes. The TSEITIN mode only introduces auxiliary variables for cross-tree constraint expressions.
...
**Both modes use this same direct encoding.** No auxiliary variables are introduced for feature tree relations.
...
**Both modes use this same direct encoding.** Cardinality clauses are emitted at arbitrary length without auxiliary variables.
...
| UVL Operator | Semantics | Straightforward CNF | Tseitin CNF |
|--------------|-----------|---------------------|-------------|
...
| `!A` | NOT | Literal negation: `¬A` | `aux ⟺ ¬A`: <br> `(aux ∨ A)` <br> `(¬aux ∨ ¬A)` |
| `A => B` | IMPLIES | Rewrite as `(¬A ∨ B)` | `aux ⟺ (A → B)`: <br> `(¬aux ∨ ¬A ∨ B)` <br> `(aux ∨ A)` <br> `(aux ∨ ¬B)` |
| `A <=> B` | IFF | Rewrite as `(A → B) ∧ (B → A)` | `aux ⟺ (A ↔ B)`: <br> `(¬aux ∨ A ∨ ¬B)` <br> `(¬aux ∨ ¬A ∨ B)` <br> `(aux ∨ A ∨ B)` <br> `(aux ∨ ¬A ∨ ¬B)` |
| `A requires B` | REQUIRES | Same as `A => B` | Same as IMPLIES |
| `A excludes B` | EXCLUDES | `(¬A ∨ ¬B)` | Same |
...
## Tseitin Encoding for Cross-Tree Constraints

In Tseitin mode, **auxiliary variables** are introduced for each Boolean sub-expression in cross-tree constraints. This prevents exponential clause growth that can occur with the straightforward distribution-law approach for deeply nested formulas.

Feature tree relation clauses (OR groups, ALTERNATIVE groups, CARDINALITY) are **not** decomposed — they are always emitted directly as CNF disjunctions of arbitrary length.

### Binary Operator Encodings

Each Boolean operator in a cross-tree constraint is encoded using a full biconditional equivalence between an auxiliary variable and the operator's result:
...
**All three clauses are required** for the full equivalence, ensuring `aux` is true **if and only if** at least one of `v₁, v₂` is true.
...
### Complexity for Cross-Tree Constraints

For a cross-tree constraint with n binary sub-expressions:
- **Straightforward:** May grow exponentially (distribution law)
- **Tseitin:** O(n) auxiliary variables, O(n) clauses

### Where Tseitin Encoding Applies

Auxiliary variables are introduced **only** for:
- Boolean cross-tree constraint expressions (AND, OR, NOT, IMPLIES, IFF) with nested operations

They are **not** introduced for:
- Feature tree relations (OR groups, ALTERNATIVE groups, CARDINALITY) — emitted directly
...
**Expected output:**
```
[1/20] Testing model1... PASS (solutions: 42)
[2/20] Testing model2... PASS (solutions: 128)
...
✓ All tests passed!

This confirms that the Tseitin transformation:
  1. Uses full biconditional equivalences (⟺)
  2. Preserves solution counts exactly
```
...
- ✅ **Tseitin mode:** Simplifies Tseitin encoding (including auxiliary variables)
...
**Tseitin CNF:** Same as straightforward — feature tree relation clauses are emitted directly without decomposition. No auxiliary variables are introduced for OR groups.
...
|--------|----------------|---------|-----------------|
| **Variables** | Fewer (features only) | More (features + auxiliaries for cross-tree constraints) | Unchanged (same as base mode) |
| **Clauses** | Fewer total | More total | **30-50% reduction** |
| **Feature tree relation clauses** | Direct (arbitrary length) | Direct (arbitrary length) | Same as base mode |
| **Cross-tree constraint clauses** | May grow exponentially | Linear size | Same as base mode |
| **Solution Counts** | ✅ Preserved | ✅ Preserved (via full equivalences) | ✅ Preserved |
| **Best for** | General use | Deeply nested cross-tree constraints | Storage, distribution, reuse |
| **Technique** | NNF + distribution | Tseitin auxiliary variables for constraints | SAT-based backbone detection + simplification |
| **Auxiliary Variables** | None | One per Boolean sub-expression in constraints | N/A (works with both modes) |
| **Overhead** | None | Minimal (more clauses for constraint-heavy models) | 50-200ms (backbone computation) |

### Key Takeaways

1. **Both modes are semantically equivalent** - they preserve all solutions
2. **Tseitin prevents exponential clause growth** in cross-tree constraints through:
   - Introducing auxiliary variables for sub-expressions
   - Using **full biconditional equivalences** (⟺), not implications (→)
   - Feature tree relation clauses are always emitted directly (no decomposition)
