...
void FeatureModelBuilder::enterFeature(UVLCppParser::FeatureContext *ctx) {
    ...
    // Feature cardinality declares a multi-instance feature (the feature may be
    // instantiated several times). This is outside the Boolean language level that
    // uvl2dimacs targets: representing the instantiation count would require numeric
    // variables. Warn and ignore the annotation, treating the feature as a single
    // Boolean variable. (This is distinct from group cardinality, which is supported.)
    if (ctx->featureCardinality()) {
        std::string cardinality_text;
        if (ctx->featureCardinality()->CARDINALITY()) {
            cardinality_text = " " + ctx->featureCardinality()->CARDINALITY()->getText();
        }
        std::cerr << "Warning: feature \"" << feature_name
                  << "\" declares a feature cardinality" << cardinality_text
                  << " (a multi-instance feature), which is outside the Boolean"
                     " language level supported by uvl2dimacs; the cardinality is"
                     " ignored and the feature is treated as a single Boolean feature."
                  << std::endl;
    }
}
