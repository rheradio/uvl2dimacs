...
# Verify sharpSAT can actually execute on this machine (architecture / Gatekeeper check)
echo -n "Verifying SharpSAT-TD runs on this platform... "
_TRIVIAL="$TEMP_DIR/trivial_check_$$.dimacs"
printf "p cnf 1 1\n1 0\n" > "$_TRIVIAL"
_SHARPSAT_OUT=$(cd "$SHARPSAT_DIR/bin" && ./sharpSAT $SHARPSAT_DECOT_FLAG -tmpdir "$TEMP_DIR" "$_TRIVIAL" 2>&1)
_SHARPSAT_RC=$?
rm -f "$_TRIVIAL"
if [ $_SHARPSAT_RC -ne 0 ] || ! echo "$_SHARPSAT_OUT" | grep -q "c s exact arb int"; then
    echo -e "${RED}FAILED${NC}"
    echo ""
    echo -e "${RED}SharpSAT-TD cannot run on this machine.${NC}"
    if [ -n "$_SHARPSAT_OUT" ]; then
        echo "SharpSAT output:"
        echo "$_SHARPSAT_OUT" | head -10
        echo ""
    fi
    if [[ "$(uname -s)" == "Darwin" ]]; then
        _BIN_ARCH=$(file "$SHARPSAT" 2>/dev/null | grep -o "arm64\|x86_64" | head -1)
        _MACHINE_ARCH=$(uname -m)
        if [ "$_BIN_ARCH" != "$_MACHINE_ARCH" ]; then
            echo -e "${YELLOW}Architecture mismatch: binary is $_BIN_ARCH but this machine is $_MACHINE_ARCH.${NC}"
        else
            echo -e "${YELLOW}Possible macOS Gatekeeper quarantine. Try:${NC}"
            echo "    xattr -d com.apple.quarantine \"$SHARPSAT\""
            echo "    xattr -d com.apple.quarantine \"$SHARPSAT_DIR/bin/flow_cutter_pace17\""
            echo ""
        fi
        echo -e "${GREEN}To rebuild SharpSAT-TD from source for this platform:${NC}"
        echo "    cd \"$SHARPSAT_DIR\""
        echo "    mkdir -p build && cd build"
        echo "    cmake -DCMAKE_BUILD_TYPE=Release .."
...
count_solutions() {
    ...
    local output=$(./sharpSAT $SHARPSAT_DECOT_FLAG -tmpdir "$TEMP_DIR" "$abs_dimacs_file" 2>&1)
    local exit_code=$?
    cd - > /dev/null
    ...
    local count=$(echo "$output" | grep "c s exact arb int" | { read -r line; echo "${line##* }"; })

    if [ -z "$count" ]; then
        echo "ERROR"
        echo "  [sharpSAT output: $(echo "$output" | head -3 | tr '\n' ' ')]" >&2
        return 1
    fi
...
echo "============================================================"
echo "Testing Tseitin Transformation (Solution Counting)"
echo "============================================================"
echo ""
echo -e "${BLUE}This test verifies that the Tseitin transformation:"
echo -e "Preserves solution counts using full equivalences (⟺)${NC}"
echo ""
echo "CLI: $CLI"
...
# Test parameters: optional first argument limits the number of models tested
NUM_TESTS="${1:-}"

# Get list of UVL files sorted by size (smallest first for faster testing)
all_uvl_files=($(find "$UVL_DIR" -name "*.uvl" -exec wc -l {} \; 2>/dev/null | sort -n | while read -r _count _file; do echo "$_file"; done))

if [ ${#all_uvl_files[@]} -eq 0 ]; then
    echo -e "${RED}Error: No UVL files found in $UVL_DIR${NC}"
    exit 1
fi

if [ -n "$NUM_TESTS" ]; then
    uvl_files=("${all_uvl_files[@]:0:$NUM_TESTS}")
    echo "Selecting $NUM_TESTS models (preferring smaller ones for speed)..."
else
    uvl_files=("${all_uvl_files[@]}")
    echo "Selecting all ${#uvl_files[@]} models (sorted by size)..."
fi
...
for uvl_file in "${uvl_files[@]}"; do
    ...
    if [ "$count_straight" = "UNSAT" ] || [ "$count_tseitin" = "UNSAT" ]; then
        if [ "$count_straight" = "UNSAT" ] && [ "$count_tseitin" = "UNSAT" ]; then
            echo -e "${GREEN}PASS (both UNSAT)${NC}"
            ((passed++))
        else
    ...
    if [ "$count_straight" = "$count_tseitin" ]; then
        echo -e "${GREEN}PASS (solutions: $count_straight)${NC}"
        ((passed++))
    else
        ...
        straight_vars=$(grep "^p cnf" "$straight_dimacs" | { read -r _ _ v _; echo "$v"; })
        tseitin_vars=$(grep "^p cnf" "$tseitin_dimacs" | { read -r _ _ v _; echo "$v"; })
...
if [ $failed -eq 0 ] && [ $passed -gt 0 ]; then
    ...
    echo "  1. Uses full biconditional equivalences (⟺)"
    echo "  2. Preserves solution counts exactly"
    echo ""
